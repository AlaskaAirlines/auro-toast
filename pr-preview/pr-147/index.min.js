...
class AuroToast extends i$2 {
  ...
  updated(changedProperties) {
    if (changedProperties.has("visible")) {
      ...
      // Unconditional, not gated on `!this.visible`: Lit only records a
      // property's first old value per update batch, so a synchronous
      // `visible = false; visible = true;` (no await between them) still
      // reports changedProperties.has("visible") but `this.visible` reads
      // the final (true) value here — a `!this.visible` guard would miss
      // it entirely. Cancelling unconditionally is always safe: a freshly
      // re-shown toast should never carry a stale fade timer or "hidden"
      // class, and closeToast() already clears both itself when visible
      // genuinely becomes false.
      this._cancelFadeOut();
    }
